...
class Home extends Component {
	...
	handleFormSubmit = () => {
		...
		if (!isLogin) {
			...
			this.setState({
				to: '/lines/' + formDes + '/' + formType + '/' + formStartDay + '/' + formEndDay
			})
		}
	}
	...
	render() {
		const {preferencesModalOpen, isLogin, visible, activeMenu, formType, videoModalOpen, videoUrl, loginModalOpen, registerModalOpen, to} = this.state
							...
							<Form className="slogan_form">
								...
								<Form.Group inline className="slogan_form_row">
									<Form.Field>
										<label>出发时间</label>
										<DayPickerInput
											onDayChange={(day) => this.setState({formStartDay: day.getTime()})}
											dayPickerProps={{
												months: MONTHS,
									...
									<Form.Field>
										<label>返程时间</label>
										<DayPickerInput
											onDayChange={(day) => this.setState({formEndDay: day.getTime()})}
											dayPickerProps={{
												months: MONTHS,
												weekdaysLong: WEEKDAYS_LONG,
												weekdaysShort: WEEKDAYS_SHORT
