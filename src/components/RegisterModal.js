			...
			<Modal.Content>
				<Form>
					<Form.Field required>
						...
						{this.state.phoneWarning ? <p style={{'color': 'red', 'fontSize': '8px'}}>请输入有效手机号</p> :
							<p></p>}
					</Form.Field>
					<Form.Field required>
						<label>验证码</label>
						<Form.Group widths='equal'>
							...
							<Button color='green' style={{width: '200px'}} disabled={verifyBtn}
							        onClick={this.handleVerifyBtn}>{verifyNote}</Button>
						</Form.Group>
					</Form.Field>
					<Form.Field>
						<label>用户名</label>
						<Form.Input placeholder='请输入用户名' value={username}
					...
					<Form.Field required>
						...
						{this.state.passwordLenWarning ? <p style={{'color': 'red', 'fontSize': '8px'}}>密码最短长度为4位</p> :
							<p></p>}
					</Form.Field>
					<Form.Field required>
						<label>确认密码</label>
						<Form.Input placeholder='请重新输入密码' type='password' value={confirmPassword}
						            onBlur={() => this.setState({passwordWarning: password !== confirmPassword})}
						            onChange={(e) => {
							            this.setState({confirmPassword: e.target.value});
							            this.clearWarning();
						            }} error={passwordConfirmValid}/>
						{this.state.passwordWarning ? <p style={{'color': 'red', 'fontSize': '8px'}}>请保持输入的密码一致</p> :
							<p></p>}
					</Form.Field>
				</Form>
			</Modal.Content>
